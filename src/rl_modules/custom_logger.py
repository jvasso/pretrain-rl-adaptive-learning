...
class CustomLogger(BaseLogger):
    ...
    def init_log_file(self):
        log_dict = CustomLogger.INIT_LOG_DICT
    ...
    def log_train_data(self, collect_result: dict, step: int) -> None:
        ...
        current_episode_idx = self.last_log_train_ep
        current_step        = self.last_log_train_step
        nb_collected_episodes = collect_result["n/ep"]
        steps_per_ep_list = collect_result["lens"]
        assert np.all(steps_per_ep_list == steps_per_ep_list[0])
        steps_per_ep = steps_per_ep_list[0]
        if nb_collected_episodes > 0:
            for ep in range(collect_result["n/ep"]):
                current_episode_idx += 1
                current_step        += steps_per_ep
                if current_episode_idx - self.last_log_train_ep >= self.train_interval_ep:
                    log_data = {
                        "ep_idx": current_episode_idx,
                        "step":current_step,
                        "rew": collect_result["rews"][ep],
                        "length": collect_result["lens"][ep],
                    }
                    self.write("train", step, log_data)
                    self.last_log_train_ep   = current_episode_idx
                    self.last_log_train_step = current_step
